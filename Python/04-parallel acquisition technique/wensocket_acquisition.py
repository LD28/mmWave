...
sweep_mat_dup = []
distance = np.linspace(200, 600, 620)

# async def send_data():
#     async with websockets.connect("ws://localhost:8785") as websocket:
#         #n=random_gen()
...
def module_software_test(port, flowcontrol, streaming, duration):
    ...
    if streaming:
        ...
        com.register_write(0x23, 150000)
...
#             print(str(dummy))
    ...
    ### IO operation ###
                ...
                if e.errno == errno.EPIPE:
                    ws.close()
                    #create_connection("ws://localhost:8847")
    ...
    com.register_write(0x03, 0)
dummy=np.arange(100,700).tolist()
import websocket
from websocket import create_connection
ws = create_connection("ws://localhost:8133")
def return_data():
    pass
    return data_buffer 
...
def gesture_prediction():
    ...
    while True:
        ...
        elif np.argmax(model.predict(gesture))==2:
            print('Hand close')
        elif np.argmax(model.predict(gesture))==3:
            print('No gesture')
        else:
            print('Swipe left')
        '''
